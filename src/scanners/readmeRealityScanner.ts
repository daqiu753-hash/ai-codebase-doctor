...
type PackageCommand = { raw: string; script: string }
...
export function extractPackageCommands(markdown: string): PackageCommand[] {
  ...
  for (const line of markdown.split(/\r?\n/)) {
    for (const segment of splitShellSegments(line)) {
      const command = parsePackageCommand(segment)
...
function parsePackageCommand(segment: string): PackageCommand | undefined {
  ...
  return { raw: shorthand[0].trim(), script }
}
